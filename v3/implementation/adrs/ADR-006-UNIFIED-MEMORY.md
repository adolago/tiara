...
### Package Version
- `@tiara/memory@3.0.0-alpha.2` (published 2026-01-07)
...
Added `memory init` command to CLI (`@tiara/cli@3.0.0-alpha.56`) using **sql.js** (WASM SQLite) for cross-platform compatibility without native compilation.

```bash
# Initialize memory database
npx @tiara/cli@latest memory init

# Options
npx @tiara/cli@latest memory init --backend sqlite  # Default
npx @tiara/cli@latest memory init --path ./data/custom.db
npx @tiara/cli@latest memory init --force  # Overwrite existing
```
