...
function generateEnhancedMarkdown(report: FullValidationReport, options: ReportOptions): string {
  ...
  lines.push('|-----------|-----------|');
  lines.push('| claude-flow/hive-mind | @tiara/swarm |');
  lines.push('| claude-flow/swarm | @tiara/swarm |');
  lines.push('| claude-flow/memory | @tiara/memory |');
  lines.push('| claude-flow/agents | @tiara/agent-lifecycle |');
  lines.push('| claude-flow/tasks | @tiara/task-execution |');
  lines.push('');
...
function generateExamplesSection(): string[] {
  ...
  lines.push('npx @tiara/cli swarm init');
  lines.push('npx @tiara/cli swarm status');
  lines.push('```');
  lines.push('');
  ...
  lines.push("import { UnifiedSwarmCoordinator as HiveMind } from '@tiara/swarm';");
  lines.push("import { UnifiedMemoryService as MemoryManager } from '@tiara/memory';");
  lines.push('');
...
function generateMigrationScriptsSection(): string[] {
  ...
  lines.push('npx @tiara/cli migrate --from v2 --to v3');
  lines.push('');
  lines.push('# Migrate configuration');
  lines.push('npx @tiara/cli migrate config --input .claude-flow/config.yaml');
  lines.push('');
  lines.push('# Migrate memory database');
  lines.push('npx @tiara/cli migrate memory --input .claude-flow/memory.db');
  lines.push('```');
  lines.push('');
