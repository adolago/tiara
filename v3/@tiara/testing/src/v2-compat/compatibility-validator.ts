...
export class V2CompatibilityValidator {
  ...
  async validateAPI(): Promise<ValidationResult> {
    ...
    for (const iface of V2_API_INTERFACES) {
      ...
      checks.push({
        ...
        v3Behavior: isSupported
          ? `Import "${v3Equivalent}" from @tiara/*`
          : 'Not available',
  ...
  private generateRecommendations(
    ...
    if (cli.breakingChanges > 0) {
      ...
      recommendations.push('Run migration script: npx @tiara/cli migrate');
    }
    ...
    if (api.breakingChanges > 0) {
      recommendations.push('Update import statements to use @tiara/* packages');
...
export function generateCompatibilityReport(report: FullValidationReport): string {
  ...
  lines.push("import { UnifiedSwarmCoordinator as SwarmCoordinator } from '@tiara/swarm';");
  lines.push('```');
  lines.push('');
  lines.push('---');
  lines.push('');
