...
export function createV3ProgressWorker(projectRoot: string): WorkerHandler {
  return async (): Promise<WorkerResult> => {
    const startTime = Date.now();
    const v3Path = path.join(projectRoot, 'v3');
    const cliPath = path.join(v3Path, '@tiara', 'cli', 'src');
    ...
    // Count @tiara packages (excluding hidden directories)
    let packages = 0;
    const packageDirs: string[] = [];
    try {
      const packagesPath = path.join(v3Path, '@tiara');
    ...
    for (const pkg of packageDirs) {
      ...
      try {
        const srcPath = path.join(v3Path, '@tiara', pkg, 'src');
    ...
    let totalLines = 0;
    try {
      const v3ClaudeFlow = path.join(v3Path, '@tiara');
