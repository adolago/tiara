 ...
 * - ONNX embeddings via @tiara/embeddings (MiniLM-L6 384-dim)
 * - AgentDB backend for persistence
 * - Pattern promotion from short-term to long-term memory
 *
 * @module @tiara/hooks/reasoningbank
 */
...
export class ReasoningBank extends EventEmitter {
  ...
  private async loadDependencies(): Promise<void> {
    ...
    const memoryModule = await dynamicImport('@tiara/memory');
    if (memoryModule) {
      AgentDBAdapter = memoryModule.AgentDBAdapter;
      HNSWIndex = memoryModule.HNSWIndex;
    }

    const embeddingsModule = await dynamicImport('@tiara/embeddings');
    if (embeddingsModule) {
...
/**
 * Real embedding service using @tiara/embeddings
 */
