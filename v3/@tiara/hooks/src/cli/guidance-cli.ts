 ...
 *
 * Usage:
 *   npx @tiara/hooks session-context
 *   npx @tiara/hooks user-prompt "Fix authentication bug"
 *   npx @tiara/hooks pre-edit "/path/to/file.ts"
 *   npx @tiara/hooks route "Implement caching layer"
 *
 * @module @tiara/hooks/cli/guidance-cli
 */
...
function printHelp(): void {
  console.log(`
@tiara/hooks - V3 Guidance System CLI

Usage: npx @tiara/hooks <command> [args]
...
Examples:
  ...
  npx @tiara/hooks user-prompt "Fix authentication security vulnerability"

  # Pre-edit security check
  npx @tiara/hooks pre-edit "src/auth/login.ts"

  # Agent routing
  npx @tiara/hooks route "Implement HNSW vector search"

  # Store a learned pattern
  npx @tiara/hooks store "Use dependency injection for testability" architecture

  # Broadcast pattern to swarm
  npx @tiara/hooks swarm-pattern-broadcast "Use HNSW for 150x faster search" memory

  # Initiate consensus
  npx @tiara/hooks swarm-consensus "Which auth method?" "JWT,OAuth2,Session"

  # Hand off task
  npx @tiara/hooks swarm-handoff security-auditor "Review auth implementation"
`);
}
