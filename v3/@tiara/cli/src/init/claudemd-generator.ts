...
Bash("npx @tiara/cli@latest swarm init --topology ${topology} --max-agents ${maxAgents}")
...
Bash("npx @tiara/cli@latest memory search --query '[task keywords]' --namespace patterns")

# 2. Check if similar task was done before
Bash("npx @tiara/cli@latest memory search --query '[task type]' --namespace tasks")

# 3. Load learned optimizations
Bash("npx @tiara/cli@latest hooks route --task '[task description]'")
\`\`\`

### After Completing Any Task Successfully
\`\`\`bash
# 1. Store successful pattern for future reference
Bash("npx @tiara/cli@latest memory store --namespace patterns --key '[pattern-name]' --value '[what worked]'")

# 2. Train neural patterns on the successful approach
Bash("npx @tiara/cli@latest hooks post-edit --file '[main-file]' --train-neural true")

# 3. Record task completion with metrics
Bash("npx @tiara/cli@latest hooks post-task --task-id '[id]' --success true --store-results true")

# 4. Trigger optimization worker if performance-related
Bash("npx @tiara/cli@latest hooks worker dispatch --trigger optimize")
\`\`\`
...
npx @tiara/cli@latest memory search --query "authentication patterns"
...
npx @tiara/cli@latest hooks post-edit --file "[file]" --train-neural true
...
npx @tiara/cli@latest hooks coverage-gaps --format table
npx @tiara/cli@latest hooks coverage-route --task "[task]"

# Statusline (for Claude Code integration)
npx @tiara/cli@latest hooks statusline
npx @tiara/cli@latest hooks statusline --json
\`\`\`
...
npx @tiara/cli@latest migrate status

# Run migration with backup
npx @tiara/cli@latest migrate run --backup

# Rollback if needed
npx @tiara/cli@latest migrate rollback

# Validate migration
npx @tiara/cli@latest migrate validate
\`\`\`
...
Bash("npx @tiara/cli@latest hooks post-command --command '[operation]' --track-metrics true")

# Periodically run benchmarks (every major feature)
Bash("npx @tiara/cli@latest performance benchmark --suite all")

# Analyze bottlenecks when performance degrades
Bash("npx @tiara/cli@latest performance profile --target '[component]'")
\`\`\`

### Session Persistence (Cross-Conversation Learning)
\`\`\`bash
# At session start - restore previous context
Bash("npx @tiara/cli@latest session restore --latest")

# At session end - persist learned patterns
Bash("npx @tiara/cli@latest hooks session-end --generate-summary true --persist-state true --export-metrics true")
\`\`\`

### Neural Pattern Training
\`\`\`bash
# Train on successful code patterns
Bash("npx @tiara/cli@latest neural train --pattern-type coordination --epochs 10")

# Predict optimal approach for new tasks
Bash("npx @tiara/cli@latest neural predict --input '[task description]'")

# View learned patterns
Bash("npx @tiara/cli@latest neural patterns --list")
\`\`\`
...
claude mcp add claude-flow -- npx -y @tiara/cli@latest
...
- **Memory store**: \`npx @tiara/cli@latest memory store --key "mykey" --value "myvalue" --namespace patterns\`
- **Memory search**: \`npx @tiara/cli@latest memory search --query "search terms"\`
- **Memory list**: \`npx @tiara/cli@latest memory list --namespace patterns\`
- **Memory retrieve**: \`npx @tiara/cli@latest memory retrieve --key "mykey" --namespace patterns\`
...
npx @tiara/cli@latest memory store --key "pattern-auth" --value "JWT with refresh tokens" --namespace patterns
npx @tiara/cli@latest memory store --key "bug-fix-123" --value "Fixed null check" --namespace solutions --tags "bugfix,auth"
\`\`\`
...
npx @tiara/cli@latest memory search --query "authentication patterns"
npx @tiara/cli@latest memory search --query "error handling" --namespace patterns --limit 5
\`\`\`

### List Entries
\`\`\`bash
# OPTIONAL: --namespace, --limit
npx @tiara/cli@latest memory list
npx @tiara/cli@latest memory list --namespace patterns --limit 10
\`\`\`
...
npx @tiara/cli@latest memory retrieve --key "pattern-auth"
npx @tiara/cli@latest memory retrieve --key "pattern-auth" --namespace patterns
\`\`\`

### Initialize Memory Database
\`\`\`bash
npx @tiara/cli@latest memory init --force --verbose
\`\`\`
...
# Core
npx @tiara/cli@latest init --wizard
npx @tiara/cli@latest agent spawn -t coder
npx @tiara/cli@latest swarm init --v3-mode
npx @tiara/cli@latest memory search --query "query"

# Advanced
npx @tiara/cli@latest daemon start
npx @tiara/cli@latest doctor --fix
npx @tiara/cli@latest security scan
npx @tiara/cli@latest performance benchmark
\`\`\`
