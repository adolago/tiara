...
function generateStatusLineConfig(options: InitOptions): object {
  ...
  // Uses npx @tiara/cli@latest (or @alpha) to run the hooks statusline command
  ...
  const statuslineCommand = 'npx @tiara/cli@latest hooks statusline 2>/dev/null || node .claude/helpers/statusline.cjs 2>/dev/null || echo "▊ Claude Flow V3"';
    ...
    hooks.PreToolUse = [
      // File edit hooks with intelligence routing
      {
        ...
        hooks: [
          {
            type: 'command',
            command: '[ -n "$TOOL_INPUT_file_path" ] && npx @tiara/cli@latest hooks pre-edit --file "$TOOL_INPUT_file_path" 2>/dev/null || true',
      ...
      // Bash command hooks with safety validation
      {
        ...
        hooks: [
          {
            type: 'command',
            command: '[ -n "$TOOL_INPUT_command" ] && npx @tiara/cli@latest hooks pre-command --command "$TOOL_INPUT_command" 2>/dev/null || true',
            timeout: config.timeout,
            continueOnError: true,
          },
        ],
      ...
      // Task/Agent hooks - require task-id for tracking
      {
        ...
        hooks: [
          {
            type: 'command',
            command: '[ -n "$TOOL_INPUT_prompt" ] && npx @tiara/cli@latest hooks pre-task --task-id "task-$(date +%s)" --description "$TOOL_INPUT_prompt" 2>/dev/null || true',
            timeout: config.timeout,
            continueOnError: true,
          },
        ],
    ...
    hooks.PostToolUse = [
      // File edit hooks with neural pattern training
      {
        ...
        hooks: [
          {
            type: 'command',
            command: '[ -n "$TOOL_INPUT_file_path" ] && npx @tiara/cli@latest hooks post-edit --file "$TOOL_INPUT_file_path" --success "${TOOL_SUCCESS:-true}" 2>/dev/null || true',
            timeout: config.timeout,
            continueOnError: true,
          },
        ],
      ...
      // Bash command hooks with metrics tracking
      {
        ...
        hooks: [
          {
            type: 'command',
            command: '[ -n "$TOOL_INPUT_command" ] && npx @tiara/cli@latest hooks post-command --command "$TOOL_INPUT_command" --success "${TOOL_SUCCESS:-true}" 2>/dev/null || true',
            timeout: config.timeout,
            continueOnError: true,
          },
        ],
      ...
      // Task completion hooks - use task-id
      {
        ...
        hooks: [
          {
            type: 'command',
            command: '[ -n "$TOOL_RESULT_agent_id" ] && npx @tiara/cli@latest hooks post-task --task-id "$TOOL_RESULT_agent_id" --success "${TOOL_SUCCESS:-true}" 2>/dev/null || true',
            timeout: config.timeout,
            continueOnError: true,
          },
        ],
    ...
    hooks.UserPromptSubmit = [
      {
        hooks: [
          {
            type: 'command',
            command: '[ -n "$PROMPT" ] && npx @tiara/cli@latest hooks route --task "$PROMPT" || true',
            timeout: config.timeout,
            continueOnError: true,
          },
        ],
    ...
    hooks.SessionStart = [
      {
        hooks: [
          {
            type: 'command',
            command: 'npx @tiara/cli@latest daemon start --quiet 2>/dev/null || true',
            timeout: 5000,
            continueOnError: true,
          },
          {
            type: 'command',
            command: '[ -n "$SESSION_ID" ] && npx @tiara/cli@latest hooks session-restore --session-id "$SESSION_ID" 2>/dev/null || true',
            timeout: 10000,
            continueOnError: true,
          },
        ],
    ...
    hooks.Notification = [
      {
        hooks: [
          {
            type: 'command',
            command: '[ -n "$NOTIFICATION_MESSAGE" ] && npx @tiara/cli@latest memory store --namespace notifications --key "notify-$(date +%s)" --value "$NOTIFICATION_MESSAGE" 2>/dev/null || true',
            timeout: 3000,
            continueOnError: true,
          },
        ],
