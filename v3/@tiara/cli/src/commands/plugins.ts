  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (installedOnly) {
      ...
      output.printTable({
        ...
        data: [
          { name: '@tiara/neural', version: '3.0.0', type: 'core', status: output.success('Active') },
          { name: '@tiara/security', version: '3.0.0', type: 'command', status: output.success('Active') },
          { name: '@tiara/embeddings', version: '3.0.0', type: 'core', status: output.success('Active') },
        ],
      });
...
const infoCommand: Command = {
  ...
  examples: [
    { command: 'claude-flow plugins info -n @tiara/neural', description: 'Show plugin info' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const name = ctx.flags.name as string;
    const registryName = ctx.flags.registry as string;
...
export const pluginsCommand: Command = {
  ...
  action: async (): Promise<CommandResult> => {
    ...
    output.printList([
      '@tiara/neural     - Neural patterns and inference (WASM SIMD)',
      '@tiara/security   - Security scanning and CVE detection',
      '@tiara/embeddings - Vector embeddings with hyperbolic support',
      '@tiara/claims     - Claims-based authorization',
      '@tiara/performance- Performance profiling and benchmarks',
    ]);
    output.writeln();
