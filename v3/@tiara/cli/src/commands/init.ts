    ...
    if (startDaemon || startAll) {
      ...
      if (startAll) {
        try {
          ...
          execSync('npx @tiara/cli@latest memory init 2>/dev/null', {
            stdio: 'pipe',
            cwd: ctx.cwd,
            timeout: 30000
          });
      ...
      if (startDaemon) {
        try {
          ...
          execSync('npx @tiara/cli@latest daemon start 2>/dev/null &', {
            stdio: 'pipe',
            cwd: ctx.cwd,
            timeout: 10000
          });
      ...
      if (startAll) {
        try {
          ...
          execSync('npx @tiara/cli@latest swarm init --topology hierarchical 2>/dev/null', {
            stdio: 'pipe',
            cwd: ctx.cwd,
            timeout: 30000
          });
