...
async function checkMcpServers(): Promise<HealthCheck> {
    ...
    if (existsSync(configPath)) {
      try {
        ...
        if (hasClaudeFlow) {
          ...
          return { name: 'MCP Servers', status: 'warn', message: `${count} servers (claude-flow not found)`, fix: 'claude mcp add claude-flow npx @tiara/cli@v3alpha mcp start' };
        }
      } catch {
  ...
  return { name: 'MCP Servers', status: 'warn', message: 'No MCP config found', fix: 'claude mcp add claude-flow npx @tiara/cli@v3alpha mcp start' };
}
