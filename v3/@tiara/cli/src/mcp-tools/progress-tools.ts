 ...
 *
 * @module @tiara/cli/mcp-tools/progress
 */
...
async function calculateProgress(): Promise<V3ProgressMetrics> {
  ...
  // Count V3 modules
  const modulesDir = join(V3_DIR, '@tiara');
  ...
  const commandsIndexPath = join(V3_DIR, '@tiara/cli/src/commands/index.ts');
  ...
  const toolsIndexPath = join(V3_DIR, '@tiara/cli/src/mcp-tools/index.ts');
  ...
  const hooksPath = join(V3_DIR, '@tiara/cli/src/commands/hooks.ts');
