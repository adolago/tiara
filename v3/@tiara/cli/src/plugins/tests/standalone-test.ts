...
async function main() {
  ...
  try {
    const similar = findSimilarPlugins(registry, '@tiara/neural', 3);
    console.log(`  ✅ Found ${similar.length} similar plugins to @tiara/neural`);
    passed++;
  } catch (e) {
    console.log(`  ❌ Failed: ${(e as Error).message}`);
    failed++;
  ...
  if (failed === 0) {
    ...
    console.log('    npx @tiara/cli plugins list');
    console.log('    npx @tiara/cli plugins list --official');
    console.log('    npx @tiara/cli plugins search -q "plugin creator"');
    console.log('    npx @tiara/cli plugins info -n plugin-creator');
    console.log('');
  }
