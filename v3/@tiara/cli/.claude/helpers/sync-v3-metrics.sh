...
count_modules() {
    ...
    if [ -d "$V3_DIR/@tiara" ]; then
        for dir in "$V3_DIR/@tiara"/*/; do
            if [ -d "$dir" ]; then
                name=$(basename "$dir")
                modules+=("$name")
                ((count++))
...
calculate_module_progress() {
    local module="$1"
    local module_dir="$V3_DIR/@tiara/$module"
...
check_security_status() {
    local cves_fixed=0
    local security_dir="$V3_DIR/@tiara/security/src"
...
count_domains() {
    ...
    # Map @tiara modules to DDD domains
    [ -d "$V3_DIR/@tiara/swarm" ] && ((domains++))      # task-management
    [ -d "$V3_DIR/@tiara/memory" ] && ((domains++))     # session-management
    [ -d "$V3_DIR/@tiara/performance" ] && ((domains++)) # health-monitoring
    [ -d "$V3_DIR/@tiara/cli" ] && ((domains++))        # lifecycle-management
    [ -d "$V3_DIR/@tiara/integration" ] && ((domains++)) # event-coordination
    ...
    cat > "$METRICS_DIR/v3-progress.json" << EOF
{
  "domains": {
    ...
    "total": 5,
    "list": [
      {"name": "task-management", "status": "$([ -d "$V3_DIR/@tiara/swarm" ] && echo "complete" || echo "pending")", "module": "swarm"},
      {"name": "session-management", "status": "$([ -d "$V3_DIR/@tiara/memory" ] && echo "complete" || echo "pending")", "module": "memory"},
      {"name": "health-monitoring", "status": "$([ -d "$V3_DIR/@tiara/performance" ] && echo "complete" || echo "pending")", "module": "performance"},
      {"name": "lifecycle-management", "status": "$([ -d "$V3_DIR/@tiara/cli" ] && echo "complete" || echo "pending")", "module": "cli"},
      {"name": "event-coordination", "status": "$([ -d "$V3_DIR/@tiara/integration" ] && echo "complete" || echo "pending")", "module": "integration"}
    ]
  },
  "ddd": {
  ...
  "swarm": {
    ...
    "coordination": "$([ -d "$V3_DIR/@tiara/swarm" ] && echo "ready" || echo "pending")"
  },
  ...
  "criticalVulnerabilities": [
    {
      ...
      "severity": "critical",
      "status": "$([ -f "$V3_DIR/@tiara/security/src/input-validator.ts" ] && echo "fixed" || echo "pending")",
    ...
    },
    {
      ...
      "severity": "critical",
      "status": "$([ -f "$V3_DIR/@tiara/security/src/path-validator.ts" ] && echo "fixed" || echo "pending")",
    ...
    },
    {
      ...
      "severity": "critical",
      "status": "$([ -f "$V3_DIR/@tiara/security/src/safe-executor.ts" ] && echo "fixed" || echo "pending")",
