...
Bash("npx @tiara/cli@latest swarm init --topology hierarchical-mesh --max-agents 15")
...
Bash("npx @tiara/cli@latest memory search -q '[task keywords]' --namespace patterns")

# 2. Check if similar task was done before
Bash("npx @tiara/cli@latest memory search -q '[task type]' --namespace tasks")
