 ...
 * @example
 * ```typescript
 * import { reasoningBankPlugin } from '@tiara/plugins/examples/ruvector-plugins';
...
export class ReasoningBank {
   ...
   * When set, uses @tiara/embeddings for high-quality embeddings
   */
  private embeddingProvider: ((text: string) => Promise<Float32Array>) | null = null;

  /**
   * Set external embedding provider from @tiara/embeddings
   *
   * @example
   * ```typescript
   ...
   * Used when @tiara/embeddings is not configured
   */
